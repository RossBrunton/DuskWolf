...
/** A default set of behaviours for entities
 * 
 * @name behave
 * @namespace
 * @memberof dusk.entities
 * @see dusk.entities.behave.Behave
 */
...
load.provide("dusk.entities.behave.Behave", function() {
	...
	class Behave {
		...
		constructor(entity) {
			 ...
			 * @protected
			 * @memberof! dusk.entities.behave.Behave#
			 */
			this._entity = entity;
			
			/** An event dispatcher that is fired when the entity's `{@link dusk.entities.sgui.Entity.behaviourFire}`
			 * method is called.
			 * 
			 * The properties of the event object will vary depending on it's type, but there will always be a `name`
			 *  property which contains the name of the event fired.
		 ...
		 * 
		 * Behaviour data is data that is used by a behaviours, and is shared between all of them. It is also saved and
		 * loaded with the entitiy and generally "represents" it.
		 ...
		 * @protected
		 */
		_data(name, value, init) {
			return this._entity.eProp(name, value, init);
		}
		
		/** Returns true if the specified control is active.
		 * 
		 * Other behaviours should listen for the "controlActive" event. The event object will have the property
		 *  "control", and the listeners are expected to return `true` if the control is "activated". This method will
		 *  return true if one of the listeners returns true.
		 * 
