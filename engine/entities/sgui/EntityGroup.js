...
load.provide("dusk.entities.sgui.EntityGroup", function() {
	...
	class EntityGroup extends Group {
		...
		constructor(parent, name) {
			...
			this.onPaint.listen(this._entityGroupDraw.bind(this));
		...
		//Runs in O(n) time
		getEntitiesHere(x, y, ignore, onlyOne) {
			var out = [];
		...
		getEntitiesExactlyHere(x, y, ignore, onlyOne) {
			var out = [];
			for(var c = this._entities.length-1; c >= 0; c --){
				var com = this._entities[c];
		...
		/*getCollisions(x1, y1, x2, y2, ignore, onlyOne) {
			var out = [];
			for(var c = this._entities.length-1; c >= 0; c --){
				var com = this._entities[c];
		...
		_singleMoveAndCollide(ent, noMove) {
		...
		resolveCollision(now, testee, dir, oppDir) {
