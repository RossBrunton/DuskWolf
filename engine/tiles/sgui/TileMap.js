...
load.provide("dusk.tiles.sgui.TileMap", function() {
	...
	class TileMap extends Component {
		...
		constructor(parent, name) {
			 ...
			 * @type integer
			 * @default 32
			 * @memberof! dusk.tiles.sgui.TileMap#
			 */
			this.twidth = 32;
			...
			//Listeners
			this.onPaint.listen(this._tileMapDraw.bind(this));
		 ...
		 * @since 0.0.19-alpha
		 */
		_translateAnimation(origin, offset) {
			var ani = TileMap.getAllAnimation(this.src);
			var hold = [];
			var changed = false;
		 ...
		 * @param {boolean=} exactX If true then the specified x coordinate must exactly match the x coordinate of a tile on
		 *  this map.
		 * @param {boolean=} exactY If true then the specified y coordinate must exactly match the y coordinate of a tile on
		 *  this map.
		 * @return {?array} A tileData object describing the tile, or `null`, if there is no tile here.
		 */
		tilePointIn(x, y, exactX, exactY) {
		...
		set width(value) {if(value > 0) console.warn("TileMap setting width is not supported.");}
		
		get height() {
			return this.rows*this.theight;
		}
		
		set height(value) {if(value > 0) console.warn("TileMap setting height is not supported.");}
		 ...
		 * @since 0.0.19-alpha
		 */
		static setAnimation(sheet, animation) {
		 ...
		 * @since 0.0.20-alpha
		 */
		static getAnimation(sheet, base) {
