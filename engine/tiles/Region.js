...
load.provide("dusk.tiles.Path", function() {
	...
	class Path {
		constructor(region, x, y, z, clamp) {
			this.start = [];
			this.end = [x, y, z];
			this.region = region;
...
load.provide("dusk.tiles.Region", function() {
	...
	class Region {
		constructor(cols, rows, layers, parentPool, parentNode) {
			this.rows = rows;
			this.cols = cols;
			this.layers = layers;
		...
		addTile(x, y, z, w, sw, pd, children, stop) {
		...
		expand(options) {
			...
			// And loop
			for(var e = this._removeCheapestFromCloud(cloud); e; e = this._removeCheapestFromCloud(cloud)) {
				...
				do {
					...
					if(options.children && stoppable) {
						var t = this._getFromCloud(cloud, e[0], e[1], e[2]);
				...
				for(var t of this._getAllSides(e[0], e[1], e[2], options.includeNone)) {
					...
					if(newEntry[3] <= maxRange) {
						var existing = this._getFromCloud(cloud, t[0], t[1], t[2]);
						if(existing) {
		...
		_removeCheapestFromCloud(cloud) {
			var min = -1;
		...
		_getAllSides(x, y, z, includeNone) {
			var out = [];
		...
		_calculateWeight(settings, dir, origin, dest) {
			var w = 0;
