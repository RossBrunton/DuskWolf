...
load.provide("dusk.save.SaveSpec", function() {
	...
	class SaveSpec {
		...
		save() {
			...
			var refs = [];
			var saveRef = this._saveRef.bind(this, refs);
		...
		load(saveData) {
			...
			var loaded = [];
			var loadRef = this._loadRef.bind(this, refs, loaded);
			var out = [];
		 ...
		 * @param {array.<*>} refs The array of references.
		...
		_saveRef(refs, obj) {
			...
			if(imps) {
				refs.push(obj.refSave(this._saveRef.bind(this, refs)));
				return [refs.length-1, obj.refClass()];
			}else{
				refs.push(utils.copy(obj, true));
				return refs.length-1;
			}
		}
		
		/** Loads a previously saved reference. References are generated by `{@link dusk.save.saveRef}`.
		 * @param {array<*>} The array of references.
		...
		_loadRef(refs, loaded, ref) {
			...
			if(typeof ref == "number") {
				...
				if(ref >= refs.length) throw new save.SaveIntegrityError();
				...
				if(ref[0] >= refs.length) throw new save.SaveIntegrityError();
				
				// Import package
				var o = load.evaluate(ref[1]);
				//if(!utils.doesImplement(o, save.IRefSavable)) throw new save.SaveIntegrityError();
				
				// And actually load
				loaded[ref[0]] = o.refLoad(refs[ref[0]], this._loadRef.bind(this, refs, loaded));
				return loaded[ref[0]];
			}else{
				throw new save.SaveIntegrityError();
			}
