...
load.provide("dusk.save.SaveSpec", (function() {
	 ...
	 * @since 0.0.21-alpha
	 */
	var SaveSpec = function(name, prettyName) {
	 ...
	 */
	SaveSpec.prototype.add = function(pack, type, args) {
	...
	SaveSpec.prototype.save = function() {
		...
		for(var i = this._toSave.length-1; i >= 0; i --) {
			// Get the package
			var ob = load.import(this._toSave[i][0]);
	 ...
	 * @param {array} loaded An arary used for storing the loaded refs; will be bound to an empty array by the saving
	 ...
	 * @private
	 */
	var _loadRef = function(loaded, ref) {
		...
		if(typeof ref == "number") {
			...
			// Import package
			var o = load.import(ref[1]);
			//if(!utils.doesImplement(o, save.IRefSavable)) throw new save.SaveIntegrityError();
			
			// And actually load
			loaded[ref[0]] = o.refLoad(this[ref[0]], _loadRef.bind(this, loaded));
			return loaded[ref[0]];
		}else{
			throw new save.SaveIntegrityError();
		}
