...
load.provide("dusk.items.testItems", function() {
	var Invent = load.require("dusk.items.Inventory");
	var items = load.require("dusk.items");
	
	/** Creates a bunch of test items.
	 *
	 * @memberof dusk.items
	 * @namespace
	 * @name testItems
	...
	items.items.createNewType("sword", {"itemType":"Sword"});
 	...
 	items.items.createNewType("potion", {"itemType":"Potion", "src":"pimg/items.png tiles:32x32", "tile":"0,0", "obj":{"a":1, "b":7}});
 	...
 	window.swords = new Invent(5, "(let item (== (getf item 'itemType') 'Sword'))");
 	swords.addItem(items.items.create("sword"), 5);
 	swords.addItem(items.items.create("stabbySword"), 1);
 	
 	window.extraInvent = new Invent(5, "1");
});


load.provide("dusk.items.Inventory", function() {
	var items = load.require(">dusk.items", function(p) {items = p});
	...
	class Inventory {
		 ...
		 * @param {integer=} maxStack The maximum number of items that can be stored in one
		 *  single slot. Default is Number.MAX_SAFE_INTEGER
		 */
		constructor(capacity, restriction, maxStack) {
			/** The capacity of the inventory, this is how many slots it has.
			 * @type integer
			 * @private
			 * @memberof! dusk.items.Inventory#
			 */
		 ...
		 * @return {boolean} Whether it can be added or not.
		 */
		isValidAdditionToSlot(item, slot) {
		 ...
		 *  be added.
		 * @param {integer=} amount The number of items that should be added, defaults to `1` if not specified.
		 * @param {boolean=} noFire If true, then the `itemsUpdated` event will not fire.
		 * @return {integer} The number of items that are left after all possible items were added.
		 */
		addItem(item, amount, noFire) {
		 ...
		 * @param {integer=} amount The number of items that should be removed, defaults to `1` if not specified.
		 * @param {boolean=} noFire If true, then the `itemsUpdated` event will not fire.
		 * @return {integer} The number of items that could not be removed, possibly because they do not exist.
		 */
		removeItem(type, amount, noFire) {
			if(isNaN(amount)) amount = 1;
			if(amount == 0) return 0;
		 ...
		 * @param {integer=} howMany The number of items to send. Defaults to `1` if not specified.
		 * @return {integer} The number of items that were not sent to the other inventory, and hence remain in this one.
		 */
		transferToInvent(dest, type, howMany) {
		 ...
		 * @return {integer} The number of items that were not sent to the other inventory, and hence remain in this one.
		 */
		transferToInventSlot(dest, slot, howMany) {
			if(howMany === undefined) howMany = 1;
			var type = "";
		 ...
		 * @return {integer} The number of items that were not sent to the other inventory, and hence remain in this one.
		 */
		transferSlotToInvent(dest, slot, howMany) {
			if(howMany === undefined) howMany = 1;
