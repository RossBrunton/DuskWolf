...
load.provide("dusk.utils.EventDispatcher", function() {
	...
	class EventDispatcher {
		 ...
		 * @param {integer=} filterType The current filter type, defaults to `FILTER_EQUALS`.
		 * @since 0.0.14-alpha
		 */
		constructor(name, filterType) {
			 ...
			 * @type array
			 * @private
			 * @memberof! dusk.utils.EventDispatcher#
			 */
			this._listeners = [];
		 ...
		 * @since 0.0.21-alpha
		 */
		static get FILTER_EQUALS() {return 0;}
		 ...
		 * @since 0.0.21-alpha
		 */
		static get FILTER_MULTI() {return 1;}
		 ...
		 * @since 0.0.21-alpha
		 */
		static get FILTER_ISIN() {return 2;}
		 ...
		 * 
		 * @param {function(object):*} callback The function that will be called when an event is fired. It will be
		 *  given a single argument; the event object. If you want the function to run in a scope, then you should bind
		 *   it with `bind`.
		 * @param {*} filter If defined, this must be correct with regards to the EventDispatcher's `filterType`
		 *  property.
		 ...
		 */
		listen(callback, filter, propsYes, propsNo) {
