...
load.provide("dusk.utils.functionStore", function() {
	 ...
	 * @private
	 * @memberof! dusk.utils.functionStore
	 */
	var _raws = new Map();
	...
	class _fsToken {
		 ...
		 * 
		 * It first checks the let stack to see if the token is bound by a "let" expression, if so it returns that
		 *  value, otherwise it returns the function with the appropriate name from the store.
