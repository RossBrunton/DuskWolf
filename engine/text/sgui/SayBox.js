...
load.provide("dusk.sgui.SayBox", function() {
	...
	class SayBox extends Group {
		...
		constructor (parent, name) {
			 ...
			 * @private
			 * @memberof! dusk.text.sgui.SayBox#
			 */
			...
			this.action.listen(this._action.bind(this));
			this.cancel.listen(this._cancel.bind(this));
		 ...
		 */
		say(left, body, right, context, options) {
			if(this.autoActive && !this.active) this.becomeActive();
			
			return new Promise((function(fulfill, reject) {
				if(context) {
					if(left) left = this._format(left, context);
					if(right) right = this._format(right, context);
					if(body) body = this._format(body, context);
				}
		 ...
		 * @param {string=} optionDest Currently unused.
		 * @return {object} The action
		 */
		runnerSayAction(left, body, right, vars, options, optionDest) {
