...
/** The text namespaces handles text sgui elements, as well as the tools needed to parse formated strings.
 * 
 * @name text
 * @memberof dusk
 * @namespace
...
load.provide("dusk.text.Location", function() {
	...
	class Location {
		...
		needsBreak(ctx, text) {
			...
			if(this._label.multiline
			&& this.x + this.measure(ctx, text) > this.restrictMaxWidth - this._label.padding*2) {
				return true;
			}
		 ...
		 * @param {CanvasRenderingContext2D} ctx The context to test the text with.
		 * @param {string} text The text to check.
		 * @param {integer=} base This will be added to the width of `text` before comparison, defaults to 0.
		 * @return {boolean} Whether the given text is too long to fit on a line.
		 */
		lineTooLong(ctx, text, base) {
			if(this.restrictMaxWidth < 0) return false;
			if(base === undefined) base = 0;
			
			if(this._label.multiline
			&& base + this.measure(ctx, text) > this.restrictMaxWidth - this._label.padding*2) {
				return true;
			}
		 ...
		 * @param {CanvasRenderingContext2D} ctx The context to test the text with.
		 * @param {string} text The text to check.
		 * @return {array<string>} An array of lines. The first entry will fit at the current cursor location without
		 *  being too long, while each element after it can be fit on a whole line without wrapping.
		 */
		lineWrap(ctx, text) {
...
load.provide("dusk.text.FormatBlock", function() {
	 ...
	 */
	class FormatBlock {
		/** Creates a new FormatBlock
		 *
		 * @param {array<(dusk.text.FormatBlock|string)>=} body The children of this node, as described above.
		 */
		constructor(body) {
			/** The array of children.
			 * @type array<(dusk.text.FormatBlock|string)>
			 * @protected
			 * @memberof! dusk.text.FormatBlock#
			 */
		 ...
		 * @return {integer} The number of characters that still need to be drawn. Basically, chars - (how many
		 *  characters I drew).
		 */
		 ...
		 * @param {class} block The FormatBlock to use for that tag, should extend `{@link dusk.text.FormatBlock}`.
		 */
		static register(tag, block) {
			_registered.set(tag, block);
		}
		
		/** Gets a previously registered block for the given tag.
		 * @param {string} tag The tag to look for.
		 * @return {class} block The FormatBlock for that tag, should extend `{@link dusk.text.FormatBlock}`.
		 */
		static get(tag) {
