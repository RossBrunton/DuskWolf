 ...
 * 
 * Formatting, if enabled, works using `"[name]"` tags. Generally, a `"[/name]"` version of the tag will end or reverse
 * the formatting of the unslashed equivalent. The tags supported on a label are as follows:
 * 
 ...
 * - `[img i]` Displays the image specified by the path at the correct aspect ratio, and at the same height as the text
 * the "no available character" character will be displayed while it is loading.
 * 
...
dusk.sgui.Label = function(parent, comName) {
	...
	/** The stored width of the text. This is generated before the text is drawn so we know how wide the canvas should 
	 * be. This is also used when returning the `width` property.
	 * @type integer
	 * @private
	 */
	this._cachedWidth = 0;
	 ...
	 */
	this.font = "sans";
	/** The colour of the text. This should be a CSS colour value. It can be accessed with the JSON property `"colour"`
	 * as well.
	 * @type string
	 * @default "#000000"
	 */
	 ...
	 */
	this.borderSize = 0;
	/** The amount of "padding" the text has, in pixels. This is the distance from the borders of the text box to when
	 * the actual text starts. This allows the tails and other curly bits to fit, and gives a bit of breathing room from
	 * any borders.
	 * @type integer
	 * @default 3
	 */
	 ...
	 * command, then returns an array. The first element being a "formatting constant" from this class starting with 
	 * `_EVENT_*` or null if none are to be used. The second element is text to insert at the current location if the
	 * formatting constant is null, or some argument if the constant is some value or "".
	 * 
	 * The event object contains two properties, `"command"` is the command name, in lower case. The second is `"args"`
	 * which is an array of arguments in the tag, which are space separated in the command, and different elements in
	 * this array. The first element is the command name.
	 * 
	 * This is in "Last mode" so only the last non-undefined return value will matter.
	 * @type dusk.EventDispatcher
	 * @protected
	 */
	this._command = new dusk.EventDispatcher("dusk.sgui.Label._command", dusk.EventDispatcher.MODE_LAST);
	this._command.listen(function(e){return [null, ""];}, this);
	this._command.listen(function(e){return [null, "["];}, this, {"command":"["});
	this._command.listen(function(e){return [dusk.sgui.Label._EVENT_BOLD, ""];}, this, {"command":"b"});
	this._command.listen(function(e){return [dusk.sgui.Label._EVENT_DEBOLD, ""];}, this, {"command":"/b"});
	this._command.listen(function(e){return [dusk.sgui.Label._EVENT_ITALIC, ""];}, this, {"command":"i"});
	this._command.listen(function(e){return [dusk.sgui.Label._EVENT_DEITALIC, ""];}, this, {"command":"/i"});
	
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_COLOUR, e.args[1]];
	}, this, {"command":"colour"});
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_COLOUR, this.colour];
	}, this, {"command":"/colour"});
	
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_FONT, e.args[1]];
	}, this, {"command":"font"});
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_FONT, this.font];
	}, this, {"command":"/font"});
	
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_BSIZE, e.args[1]];
	}, this, {"command":"bsize"});
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_BSIZE, this.borderSize];
	}, this, {"command":"/bsize"});
	
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_BCOLOUR, e.args[1]];
	}, this, {"command":"bcolour"});
	this._command.listen(function(e){
		return [dusk.sgui.Label._EVENT_BCOLOUR, this.borderColour];
	}, this, {"command":"/bcolour"});
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_COLOUR = 1;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_IMAGE = 2;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_DEBOLD = 4;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_ITALIC = 5;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_DEITALIC = 6;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_BSIZE = 8;
 ...
 * @protected
 */
dusk.sgui.Label._EVENT_BCOLOUR = 9;
...
dusk.sgui.Label.prototype._updateCache = function(widthOnly) {
	...
	while(charData = this._nextChar(textHold)) {
		if(charData[0] == null) {
			...
			if(charData[0] == dusk.sgui.Label._EVENT_COLOUR) {
				c.fillStyle = charData[1];
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_BCOLOUR) {
				c.strokeStyle = charData[1];
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_BSIZE) {
				c.lineWidth = charData[1];
				useBorder = charData[1] > 0;
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_BOLD && c.font.indexOf("bold") === -1) {
			...
			if(charData[0] == dusk.sgui.Label._EVENT_DEBOLD && c.font.indexOf("bold") !== -1) {
				c.font = c.font.replace("bold ", "");
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_ITALIC && c.font.indexOf("italic") === -1) {
				c.font = "italic " + c.font;
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_DEITALIC && c.font.indexOf("italic") !== -1) {
				c.font = c.font.replace("italic ", "");
			}
			
			if(charData[0] == dusk.sgui.Label._EVENT_FONT) {
 ...
 * @return {array} Formatting data. First element is a `_EVENT_*` constant or null, second is either an argument to the
 * event (if it is defined) or the string to insert (if it is null). The third element is the number of characters that
 * were "consumed", and thus can be removed from the start of the text.
 * @private
 */
dusk.sgui.Label.prototype._nextChar = function(text) {
	if(text.length <= 0) return [dusk.sgui.Label._EVENT_TERM, "", 0];
 ...
 * 
 * @classdesc A textbox is an instance of `{@link dusk.sgui.Label}` that allows the user to change the text inside it
 * when active.
 * 
 * Due to limitations, the input isn't accurate and you can only edit the end of the string. But still, what do you want
 * for a canvas text entry?
 * 
 ...
 * 
 * If the user presses a key that is bound to a direction, the direction will not be typed, and focus will try to switch
