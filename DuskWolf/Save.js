...
dusk.save.saveRef = function(obj) {
	...
	var imps = dusk.utils.doesImplement(obj, dusk.save.IRefSavableInstance);
	...
	if(imps) {
		...
		return [this._refs.length-1, obj.refClass()];
	}else{
		this._refs.push(obj);
...
dusk.save.loadRef = function(ref) {
	...
	if(typeof ref == "number") {
		if(this._refsLoaded[ref]) return this._refsLoaded[ref];
		if(ref >= this._refs.length) throw new dusk.save.SaveIntegrityError();
		
		this._refsLoaded[ref] = this._refs[ref];
		return this._refsLoaded[ref];
	}else if(Array.isArray(ref) && ref.length == 2) {
		if(this._refsLoaded[ref[0]]) return this._refsLoaded[ref[0]];
		if(ref[0] >= this._refs.length) throw new dusk.save.SaveIntegrityError();
		...
		if(!dusk.utils.doesImplement(o, dusk.save.IRefSavable)) throw new dusk.save.SaveIntegrityError();
		this._refsLoaded[ref[0]] = o.refLoad(this._refs[ref[0]]);
		return this._refsLoaded[ref[0]];
	}else{
		throw new dusk.save.SaveIntegrityError();
	}
};
...
dusk.save.SaveSpec.prototype.save = function() {
	...
	saveData.meta().refs = dusk.utils.clone(dusk.save._refs);
...
dusk.save.SaveSpec.prototype.load = function(saveData) {
	...
	for(var p in saveData.data) {
		if(p != "meta") {
			...
			if(ob) {
				if(!Array.isArray(saveData.data[p])) throw new dusk.save.SaveIntegrityError();
 ...
 */
dusk.save.IRefSavable.refLoad = function(data) {};
 ...
 */
dusk.save.IRefSavableInstance.refSave = function() {};
 ...
 */
dusk.save.IRefSavableInstance.refClass = function() {};

if("tcheckIgnore" in window) window.tcheckIgnore("dusk.save.IRefSavableInstance");

Object.seal(dusk.save.IRefSavableInstance);
 ...
 * @since 0.0.21-alpha
 */
dusk.save.SaveIntegrityError = function(parent, comName) {
	Error.call(this);
...
dusk.save.SaveIntegrityError.prototype = Object.create(Error.prototype);
